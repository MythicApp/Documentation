...
`Don't delete the game files (Delete it from Mythic only)`, this will (when toggled on) keep the game files for the game inside of `~/Applications/Games/Mythic` and show up as uninstalled in Mythic.

`Don't run uninstaller (If applicable)`, this will (if your game or app has one), run the uninstaller for it so it deletes all the related files that it installed.
